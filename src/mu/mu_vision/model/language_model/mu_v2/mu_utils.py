...
class MuAttention(nn.Module):
    def __init__(self, config, device, is_causal):
        ...
        assert self.head_dim * self.n_head == config.n_embd, "Embedding size must be divisible by n_head"
        ...
        self.out_linear = nn.Linear(config.n_embd, config.n_embd, device=device)
    ...
    def generate_sin_cos_pos_emb(self, seq_len):
        ...
        if rope_factor > 1.0:  # Apply NTK dynamic scaling
            ...
            base_adjustment = (rope_factor * seq_len_eff / max_seq_len) - (rope_factor - 1)
            adjusted_base = base * base_adjustment
            inv_freq = 1.0 / (adjusted_base ** (torch.arange(0, dim, 2, device=self.device).float() / dim))

        position_ids = torch.arange(seq_len, device=self.device, dtype=torch.float)
    ...
    def forward(self, query, key, value, mask=None, apply_rope_pos_emb=True):
        ...
        if apply_rope_pos_emb:
            cos_emb, sin_emb = self.generate_sin_cos_pos_emb(seq_len)
            torch_dtype = Q.dtype
        ...
        attn_output = attn_output.transpose(1, 2).reshape(batch_size, seq_len, self.config.n_embd)
