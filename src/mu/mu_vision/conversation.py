...
class Conversation:
    ...
    def get_prompt(self):
        ...
        if self.version == "mt5_v1":
            if "\nReference OCR token:" in messages[0][1]:
                conv_question = messages[0][1].split("\nReference OCR token:")[0]
                conv_ocr_input = messages[0][1].split("\nReference OCR token:")[1].strip()
                conv_input = "Chat | {}".format(conv_ocr_input)
                messages[0][1] = f"{conv_question}\n\n### Input:\n{conv_input}"
        ...
        if self.sep_style == SeparatorStyle.SINGLE:
            ...
            for i, (role, message) in enumerate(messages):
                if message:
                    if type(message) is tuple:
                        message, _, _ = message
                    if self.version == "mu_v1":
                        if "Reference OCR token:" in message:
                            message_question, message_ocr = message.split("Reference OCR token:")
                        ...
                        ret += role + " " + message
                        if "<image>" in ret:
                            ret = ret.replace("<image>", "")
                            ret += "\n###Context: <image>"
                        if message_ocr is not None:
                            if "\n###Context:" in ret:
                                ret += "\nReference OCR token: "
                                ret += message_ocr
                            else:
                            ...
                            # put <image> in the end.
                            if "<image>" in ret:
                                ret = ret.replace("<image>", "")
                                ret += " <image>"
                    elif self.version == "mu_v2":
                        if "Reference OCR token:" in message:
                            message_question, message_ocr = message.split("Reference OCR token:")
                            message = message_question
                        else:
...
conv_vicuna_v1 = Conversation(
    ...
    version="v1",
    messages=[["USER", "Testing Questions?"], ["ASSISTANT", "Tesing answers."]],
    offset=0,
...
conv_mt5_v1 = Conversation(
    system="You are a helpful and truthful assistant designed by Microsoft to help users with their instructions. If you cannot answer an instruction return CANNOT ANSWER. Answer the following question based on the input provided only.",
    roles=("USER", "ASSISTANT"),
    version="mt5_v1",
    messages=[["USER", "Testing Questions?"], ["ASSISTANT", "Tesing answers."]],
    offset=0,
...
conv_templates = {
    ...
    "mt5_v1": conv_mt5_v1,
    "mu_v1": conv_mu_v1,
    "mu_v2": conv_mu_v2,  # for mu v2
}


if __name__ == "__main__":
    # messages = [{'from': 'human', 'value': '<image>\nOCR this image section by section, from top to bottom, and left to right. Do not insert line breaks in the output text. If a word is split due to a line break in the image, use a space instead.'}, {'from': 'gpt', 'value': 'f NPC was manufactured in Portland, Oregon, f rom 1989 to 1991 at STC Submarine Systems, and later Alcatel Submarine Networks. The system was laid by Cable &am p; Wireless Marine on the "CS Cable Ventu re".Construction.Transatlantic cables of the 19th ce ntury consisted of an outer layer of iron a nd later steel wire wrapping India rubber wrap ping gutta-percha which surrounded a multi- stranded copper wire at the core. The por tions c losest to each shore landin g had ad ditional pr otecti ve arm our wires. Gutta-percha a natura l polymer similar to r ubber had nearly i deal pr operties f'}]
    # default_conversation.messages = messages
    # print(default_conversation.get_prompt())

    # print(conv_llava_llama_2.get_prompt())
    # print(conv_vicuna_v1.get_prompt())
    # print(conv_mt5_v1.get_prompt())

    # conv = conv_templates["mt5_v1"].copy()
    # conv.messages = []
    # conv.append_message(conv.roles[0], "What is the question?\nReference OCR token: t1, t2, t3 t4")
    # conv.append_message(conv.roles[1], None)
    # new_prompt = conv.get_prompt()
    # print(new_prompt)

    conv = conv_templates["mu_v1"].copy()
    conv.messages = []
